...
# ── Bolt TLS disabled ───────────────────────────────────────────────
# HTTPS-served Neo4j Browser requires bolt+s:// but rejects self-signed
# certs (fundamental browser WebSocket security restriction). No workaround
# exists on PaaS without a CA-signed cert for the TCP proxy hostname.
# Instead: use HTTP Transaction API, cypher-shell, or programmatic drivers.
...
# n10s bootstrap script — runs on every container start (idempotent)
COPY init-n10s.sh /scripts/init-n10s.sh
RUN sed -i 's/\r$//' /scripts/init-n10s.sh && chmod +x /scripts/init-n10s.sh

# Use init-n10s.sh as entrypoint: starts Neo4j, bootstraps n10s, then waits
ENTRYPOINT ["/scripts/init-n10s.sh"]
CMD []
